...
function ProductDetails() {
    ...
    const { id } = useParams()
    const { products, navigate, currency, addToCart, removeFromCart } = useAppContext()
    const [relatedProducts, setRelatedProducts] = useState([])
    const [thumbnail, setThumbnail] = useState(null );
    
    const product = products.find((item) => item._id === id)
    // console.log("productttt", product)
    // console.log("producttttsss", products)
    ...
    return product && (
        <div className='mt-16'>
            ...
            <div className='flex flex-col items-center mt-20'>
                ...
                <button onClick={()=>{navigate('/products'); scrollTo(0,0)}} className='text-primary  px-12 py-2.5 my-16 border rounded hover:bg-primary/10 transition cursor-pointer mx-auto'>See More</button>
            </div>
